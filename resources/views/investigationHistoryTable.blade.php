            ...
            <tbody>
                ...
                @foreach ($investigation_details as $details)
                    <tr style="font-size: 13px;">
                        ...
                        <td>
                            <a
                                href="{{ route('view_appoiment_His_details', ['channel_date' => $details->channel_date, 'patient_id' => $p->id]) }}">
        ...
        $('#generateForm').submit(function(event) {
            if (selectedRows.length === 0 && showAlert) {
                ...
                selectedRows.forEach(function(row, index) {
                    $('#generateForm').append('<input type="hidden" name="selectedRows[' +
                        index + '][date]" value="' + row.date + '">');
                    $('#generateForm').append('<input type="hidden" name="selectedRows[' +
                        index + '][categorie]" value="' + row.categorie + '">');
                    $('#generateForm').append('<input type="hidden" name="selectedRows[' +
                        index + '][details]" value="' + row.details + '">');
                    $('#generateForm').append('<input type="hidden" name="selectedRows[' +
                        index + '][treatment]" value="' + row.treatment + '">');
